...
with c1:
    ...
    ax.set_xticks([0, 1, 2, 3])  # 明確指定位置
    ax.set_xticklabels(['0', '1', '2', '3'])  # 或加描述
...
def build_radar(df, title, r_range=[-2, 2], showlegend=True):
    ...
    for cid, row in df.iterrows():
        fig.add_trace(go.Scatterpolar(
            r = row.tolist() + [row[0]],  # 首尾相連
            theta = features + [features[0]],
            name = f"Cluster {cid}",
            fill = 'toself',
            opacity = 0.4,
            line = dict(color=cluster_colors.get(cid, "#CCCCCC"))  # 預設黑色
        ))
    fig.update_layout(
        title = title,
...
# 👉 Decline level → 顧客 ID 選擇器（前面已處理好 df_final）
